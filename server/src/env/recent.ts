...
export const DEFAULT_STATE_DIR = path.join(os.homedir(), '.web-ide');
...
export async function listRecent(stateDir: string): Promise<RecentProject[]> {
...
export async function remember(
  ...
  const list = await listRecent(stateDir);
  const next = [
