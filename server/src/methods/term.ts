...
export class TermMethods {
  readonly list: Handler<'term.list'> = (_params, ctx) =>
    ctx.session.requireWorkspace().services.terminals.list();

  readonly create: Handler<'term.create'> = (params, ctx) => {
  ...
  readonly open: Handler<'term.open'> = (params, ctx) => {
  ...
  readonly attach: Handler<'term.attach'> = (params, ctx) =>
    ctx.session.requireWorkspace().services.terminals.attach(nameOf(params));

  readonly write: Handler<'term.write'> = (params, ctx) => {
  ...
  readonly resize: Handler<'term.resize'> = (params, ctx) => {
  ...
  readonly close: Handler<'term.close'> = (params, ctx) => {
    ctx.session.requireWorkspace().services.terminals.close(nameOf(params));
    return null;
  };
}

export const termMethods = new TermMethods();
