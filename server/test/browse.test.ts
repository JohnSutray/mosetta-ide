...
import { suggestDirectories } from '../src/env/browse.js';
...
describe('дерево выбора проекта', () => {
  it('без ограничения не теряет ни одной папки', async () => {
    const all = await suggestDirectories(`${home}${path.sep}`, Number.POSITIVE_INFINITY, 1);
  ...
  it('внутрь заглядывает только у первых, но показывает всех', async () => {
    const all = await suggestDirectories(`${home}${path.sep}`, Number.POSITIVE_INFINITY, 2);
    expect(all).toHaveLength(COUNT + 1);
